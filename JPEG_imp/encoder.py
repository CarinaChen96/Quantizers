...
def load_quantization_table(component):
    # Quantization Table for: Photoshop - (Save For AS10)
    if component == 'lum':
        q = np.array([[2, 2, 3, 4, 5, 6, 8, 11],
                      [2, 2, 2, 4, 5, 7, 9, 11],
                      [3, 2, 3, 5, 7, 9, 11, 12],
                      [4, 4, 5, 7, 9, 11, 12, 12],
                      [5, 5, 7, 9, 11, 12, 12, 12],
        ...
        q = np.array([[3, 3, 7, 13, 15, 15, 15, 15],
...
# define the huffman tree to do the huffman coding on the coded DC and AC component
class HuffmanTree:
    class __Node:
...
# using the quantization table to compute the quantized array
...
def main():
    ...
    # change the input image from RGB to TCbCr
